...
class StreamController extends Controller
{
    ...
    public function save(Request $request) {
        ...
        [
            ...
            'server_id.unique' => 'Поток с таким id уже существует',
            'title.required' => 'Название потока не указано',
            'genre.required' => 'Жанр не указан',
            'main_image.required' => 'Главное изображение не загружено',
            'thumbnail' => 'Иконка не загружена'
        ]);
        ...
        try {
            ...
            return response()->json(['message' => 'Поток успешно создан','status' => 'success']);
        } catch (\Exception $error) {
            return response()->json(['message' => $error->getMessage(), 'status' => 'error'], 400);
        }
    }
